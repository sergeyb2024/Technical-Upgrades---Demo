...
const { transformData } = require('../core-functions/backend-api');
...
const assembleData = async (constructorOption, raceOption) => {
  ...
  return await descriptionOfChangesForEvent(dataObjTransformed, userQueryObj.race, userQueryObj.constructor);
};
...
router.get('/', async (req, res) => {
  try {
    ...
    const dataReady = await assembleData(constructorOptionSelected, raceOptionSelected);

    res.render('pages/index', {
      constructor,
      raceName,
      textOutput: dataReady.length === 0 ? 'Make your selection' : dataReady.map((item) => (`${item}`)),
